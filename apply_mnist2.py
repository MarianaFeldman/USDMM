...
path_old_run = 'results_run64_d1'
ep_old_run = 21
WOMC_WINDOW = USDMM_data.load_window(
    path_window = f'/{work_directory}/output/{path_old_run}/run/W_V1_ep{ep_old_run}.txt',
    path_joint = f'/{work_directory}/output/{path_old_run}/run/joint_V1_ep{ep_old_run}.txt',
    path_joint_shape = f'/{work_directory}/output/{path_old_run}/run/joint_shape_V1_ep{ep_old_run}.txt'
)
...
def main():
    ...
    for i in range(4):
        path_old_run = f'results_run{run[i]}_d1'
        ep_old_run = ep[i]
        WOMC_WINDOW = USDMM_data.load_window(
            path_window = f'/{work_directory}/output/{path_old_run}/run/W_V1_ep{ep_old_run}.txt',
            path_joint = f'/{work_directory}/output/{path_old_run}/run/joint_V1_ep{ep_old_run}.txt',
        ...
        print(f'{path_old_run} - ep - {ep_old_run}')
        ...
        plt.savefig(f'/{results_dir}/confusion_matrix_run{run[i]}_ep{ep[i]}_d{1}.png')
