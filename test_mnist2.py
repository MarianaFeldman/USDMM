...
USDMM_class.WOMC_DATA(
        ...
        batch = 500, # INT -> Batch size
        path_results = 'results_run55_d1',#'results_V1_GoL', # STR -> file where we want to save the results
        name_save='_V1', # STR -> pos fixed name for the results saved
        seed = 10, #INT -> seed for reproducibilit
        early_stop_round_f = 200, #20, #INT -> max number of epochs without changes in the boolean function lattice
...
def run_experiments(base_config, results_dir, run):
    ...
    WOMC_WINDOW = USDMM_data.load_window(
        ...
        path_window = f'/{work_directory}/output/results_run53_d1/run/W_V1_ep34.txt',
        #path_joint = f'/{work_directory}/output/{path_result}/trained/joint_V{version}.txt',
        path_joint = f'/{work_directory}/output/results_run53_d1/run/joint_V1_ep34.txt',
        #path_joint_shape = f'/{work_directory}/output/{path_result}/trained/joint_shape_V{version}.txt'
        path_joint_shape = f'/{work_directory}/output/results_run53_d1/run/joint_shape_V1_ep34.txt'
    )
    ...
    USDMM_class.WOMC_DATA(
        nlayer = 7, # INT -> Number of operators in each layer
        wlen = 5,  # INT -> Size of the operator (wlen*wlen)
        train_size = 2000, # INT -> Number of images to train
        val_size = 10, # INT -> Number of images to validate
        test_size = 10, # INT -> Number of images to test
        img_type = 'mnist1',#'GoL_final', #STR ->'img_n' / 'gl' / 'classification' / 'GoL'
        error_type = 'mae', # 'mae' / 'iou' -> type of error
    ...
    #result_df_fixed.to_csv(f"{results_dir}/{path_result}_fixed2.csv", index=False)
    ...
    W_hood,w_error =  USDMM.run_window_convolve_jit(WOMC_IMG.jax_test[:3000], WOMC_IMG.jax_ytest[:3000], W_matrices, bias)
    ...
    result_df_fixed['Test_complet_error'] = w_error
    result_df_fixed.to_csv(f"{results_dir}/{path_result}_fixed3.csv", index=False)
    ...
    conf_matrix = confusion_matrix(WOMC_IMG.jax_ytest[:3000], y_pred)
    print('Matriz de Confusão:')
    print(conf_matrix)
    print("\nPredicted Labels: ", np.unique(y_pred))
    print("True Labels: ", np.unique(WOMC_IMG.jax_ytest[:3000]))
    ...
    plt.savefig(f'/{results_dir}/confusion_matrix_run{run}_d{digito}_fixed3.png')
