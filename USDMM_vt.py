...
class WOMC:
    ...
    def calculate_neighbors(self,W,  joint, k, i, Wlast, img,yimg, ep, bias):
        #print(f'Epoch {ep} - Layer {k} - Neighbor {i}')
        ...
        for ep in range(1,self.epoch_f+1):
            ...
            for k in range(self.nlayer):
                ...
                with ProcessPoolExecutor() as executor:
                    futures = [executor.submit(self.calculate_neighbors,W,  joint, k, i, Wtrain, train_b,ytrain_b,ep, bias) for i in neighbors_to_visit]
                #n_s = len(neighbors_to_visit)
                #with ProcessPoolExecutor() as executor:
                #    error_ep_f_hist = list(executor.map(self.calculate_neighbors, [W]*n_s,  [joint]*n_s, [k]*n_s, neighbors_to_visit, [Wtrain]*n_s, [train_b]*n_s,[ytrain_b]*n_s,[ep]*n_s, [bias]*n_s))
            wait(futures)

            #print('------')
            #print(error_ep_f_hist)
            
            #print(error_ep_f_hist)
            #sorted_error_ep_f_hist = sorted(error_ep_f_hist, key=lambda x: (x[0], x[2]))
            #error_min_ep, joint, _   = sorted_error_ep_f_hist[0]
            #print(f'Epoch {ep} - error: {error_min_ep}')
            #print(f'Epoch {ep} - joint: {joint}')
    ...
    def great_neighboors_parallel(self, w_line_temp_base, i, W, joint, k, ep_w,error_ep, type):
    ...
    def check_great_neighboors(self, W,joint, ep_w,error_ep):
        ...
        for k in range(self.nlayer):
            ...
            for i in nan_idx:
                error_ep = self.great_neighboors_parallel(w_line_temp_base, i, W, joint, k,ep_w,error_ep, 1)
            #with concurrent.futures.ThreadPoolExecutor() as executor:
            #    [executor.submit(self.great_neighboors_parallel,w_line_temp_base, i, W, joint, k,ep_w,error_ep, 1) for i in nan_idx]
        return error_ep
    ...
    def check_lesser_neighboors(self, W,joint, ep_w, error_ep):
        ...
        for k in range(self.nlayer):
            ...
            w_line_temp_base = W[k].copy()
            for i in nan_idx:
                error_ep = self.great_neighboors_parallel(w_line_temp_base, i, W, joint, k,ep_w,error_ep, 0)
            #with concurrent.futures.ThreadPoolExecutor() as executor:
            #    [executor.submit(self.great_neighboors_parallel,w_line_temp_base, i, W, joint, k,ep_w,error_ep, 0) for i in nan_idx]
        return error_ep
