...
import os
import cv2
import USDMM_class_C as USDMM

def load_from_file():
    with open('./data/parameters_classification.pkl', 'rb') as file:
...
WOMC = USDMM.WOMC_JAX(
    ...
    neighbors_sample = loaded_data['neighbors_sample'], # INT/False -> Number of neighbors to sort
...
def run_window_convolve_jax(sample, W_matrices,W_last, layer,bias):
    ...
    return Wsample# [:, WOMC.ci_h, WOMC.ci_w]
...
def batch_run(b,joint,joint_shape, W_matrices,W_matrices_all,train_b,ytrain_b,bias):
    W_last = jnp.zeros((WOMC.nlayer,) + train_b[b].shape)
...
def get_error_window(W, joint,joint_shape, ep_w):
    ...
    W_matrices = mult_w_matrices(W_matrices_all, joint)
    W_last = jnp.zeros((WOMC.nlayer,) + WOMC.jax_val.shape)
...
def neighboors_func(W,joint,joint_shape, ep_w, k, i):
    W_temp = jnp.array(W)
...
def check_neighboors(W,joint,joint_shape, ep_w):
    ...
    W_train_list = [] 
    W_val_list = []
    for ix, i in enumerate(WOMC.i_ix):
        w0, w1, wt, jt, jst, wtt, wvt = neighboors_func(W,joint,joint_shape, ep_w, WOMC.k_ix[ix], i)
        w_error0.append(w0)
        w_error1.append(w1)
        W_temp.append(wt)
        joint_temp.append(jt)
        joint_shape_temp.append(jst)
        W_train_list.append(wtt)
        W_val_list.append(wvt)
    ix_error = jnp.argmin(jnp.array(w_error1))
    W_error = jnp.array([w_error0[ix_error], w_error1[ix_error]])
    W_min = W_temp[ix_error]
    joint_min = joint_temp[ix_error]
    joint_shape_min = joint_shape_temp[ix_error]
...
def _save_results_in(W, img_type, k, ep = None):
    ...
    for img in range(1,len(W_img)+1):
        ...
        x[(x==-1)]=255
        x[(x==1)]=0
        cv2.imwrite(f'{WOMC.path_results}/run/{img_type}_op{k+1}_{img:02d}{WOMC.name_save}_{ep}.jpg', x)
...
def _save_results(W, img_type, k):
    ...
    for img in range(1,len(W_img)+1):
        ...
        x[(x==-1)]=255
        x[(x==1)]=0
        cv2.imwrite(f'{WOMC.path_results}/trained/{img_type}_op{k+1}_{img:02d}{WOMC.name_save}.jpg', x)
...
def save_window_in(joint,joint_shape, W, ep):
  filename_joint = f'{WOMC.path_results}/run/joint{WOMC.name_save}_ep{ep}.txt'
  pickle.dump(joint, open(filename_joint, 'wb'))
  filename_joint_shape = f'{WOMC.path_results}/run/joint_shape{WOMC.name_save}_ep{ep}.txt'
  pickle.dump(joint_shape, open(filename_joint_shape, 'wb'))
  filename_W =f'{WOMC.path_results}/run/W{WOMC.name_save}_ep{ep}.txt'
  pickle.dump(W, open(filename_W, 'wb'))  

def save_window(joint,joint_shape, W):
  filename_joint = f'{WOMC.path_results}/trained/joint{WOMC.name_save}.txt'
  pickle.dump(joint, open(filename_joint, 'wb'))
  filename_joint_shape = f'{WOMC.path_results}/trained/joint_shape{WOMC.name_save}.txt'
  pickle.dump(joint_shape, open(filename_joint_shape, 'wb'))
  filename_W =f'{WOMC.path_results}/trained/W{WOMC.name_save}.txt'
  pickle.dump(W, open(filename_W, 'wb')) 
