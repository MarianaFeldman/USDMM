...
class WOMC:
    ...
    def __init__(self, new, nlayer, wlen, train_size, val_size, test_size, error_type, 
        ...
        self.cp_val = cp.array(self.val)
        self.cp_test = cp.array(self.test)
        self.cp_ytrain = cp.array(self.ytrain)
        self.cp_yval = cp.array(self.yval)
        self.cp_ytest = cp.array(self.ytest)
    ...
    def create_w_matrices2(self, W, joint):
        ...
        for k in range(self.nlayer):
            ...
            matrices.append(cp.array(matrix_k))
        return matrices
    ...
    def get_batches(self, imgX,imgY, batch_size, img_size):
        cp.random.shuffle(imgX)
        cp.random.shuffle(imgY)
        num_batches = img_size // batch_size

        imgX_batches = cp.array_split(imgX, num_batches)
        imgY_batches = cp.array_split(imgY, num_batches)
    ...
    def run_window_convolve(self, sample, sample_size, W_matrices, Wlast, layer, bias):
        ...
        #for k in range(sample_size):
           ...
           #cupy_kernel = cp.array(W_matrices[i])
            ...
            Wsample_b = cp.pad(cp.array(Wsample_k), ((0, 0), (self.increase, self.increase), (self.increase, self.increase)), mode='constant', constant_values=-1)
    ...
    def calculate_neighbors(self,W,  joint, k, Wlast, img,yimg, bias, i):
        ...
        return [error_hood, cp.array(joint_temp), str(k)+str(i)]
    ...
    def get_error_window_gpu(self,W, joint, ep_w):
        ...
        if self.batch>=self.train_size:
            ...
            ytrain_b = [copy.deepcopy(self.cp_ytrain)]
        flg = 0
        epoch_min = 0
        W_size = [np.count_nonzero(layer == 1) for layer in W]
        Wtrain,w_error =  self.window_error_generate_c(W_matrices, self.cp_train, self.train_size,self.cp_ytrain, self.error_type, 0, 0, bias)
        self.time_test = {"ep":[],"time":[]}
        ...
        for ep in range(self.epoch_f):
            start_time = time()
            if self.batch<self.train_size:
                train_b, ytrain_b = self.get_batches(self.cp_train,self.cp_ytrain, self.batch, self.train_size)
            for b in range(self.num_batches):
                ...
                for k in range(self.nlayer):
                    ...
                    for nv in neighbors_to_visit: 
                       self.calculate_neighbors2(W,  joint, k, Wtrain, train_b[b],ytrain_b[b],bias, nv)

                #error_ep_f_hist_np = cp.array(error_ep_f_hist, dtype=object)
            
                #ix = cp.lexsort((error_ep_f_hist_np[:, 2], error_ep_f_hist_np[:, 0]))[0]
                #_,joint, _ = error_ep_f_hist_np[ix]
                #error_min_ep = min(self.error_ep_f_hist['error'])     
                errors = cp.array(self.error_ep_f_hist["error"])
                min_error_index = cp.argmin(errors).item()
                joint = self.error_ep_f_hist["joint"][min_error_index]       

            W_matrices = self.create_w_matrices(W, joint)
            Wtrain_min,w_error_min =  self.window_error_generate_c(W_matrices, self.cp_train, self.train_size,self.cp_ytrain, self.error_type, 0, 0, bias)
            self.error_ep_f["W_key"].append(self.windows_visit) 
            self.error_ep_f["epoch_w"].append(ep_w) 
            self.error_ep_f["epoch_f"].append(ep) 
            self.error_ep_f["error"].append(w_error_min) 
        ...
        if flg==1:
            joint = copy.deepcopy(joint_min)
        W_matrices = self.create_w_matrices(W, joint)
        _,error_val =  self.window_error_generate_c(W_matrices, self.cp_val, self.val_size, self.cp_yval, self.error_type, self.val, 0, bias)
        error = cp.array([w_error, error_val])
        return (joint, error, epoch_min)
