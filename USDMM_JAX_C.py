...
def apply_convolve_jax(img, W_matrices, bias):
    ...
    return img_c.astype(jnp.int8) #[WOMC.increase:img_c.shape[0]-WOMC.increase, WOMC.increase:img_c.shape[1]-WOMC.increase]
...
def run_window_convolve_jax_(sample, W_matrices,W_last, layer,bias):
    ...
    Wsample_k = copy.deepcopy(sample)
    for i in range(WOMC.nlayer):
        carry_forry = (W_last[i] ,W_matrices[i], bias[i], Wsample_k )
    ...
    return Wsample_k[:, WOMC.ci_h, WOMC.ci_w].astype(jnp.int8)# #Wsample.astype(jnp.int8)# [:, WOMC.ci_h, WOMC.ci_w]

def run_window_convolve_jax(sample, W_matrices, layer,bias):
    #sample_b = jnp.pad(sample, ((0, 0), (WOMC.increase, WOMC.increase), (WOMC.increase, WOMC.increase)), mode='constant', constant_values=-1)
    #Wsample=[]
    Wsample_k = copy.deepcopy(sample)
    for i in range(WOMC.nlayer):
        Wsample_k = apply_convolve_vmap(Wsample_k, W_matrices[i], bias[i]).astype(jnp.int8)
        #sample_b = jnp.pad(jnp.array(Wsample_k), ((0, 0), (WOMC.increase, WOMC.increase), (WOMC.increase, WOMC.increase)), mode='constant', constant_values=-1)
        #Wsample.append(Wsample_k)
    return Wsample_k[:, WOMC.ci_h, WOMC.ci_w].astype(jnp.int8)

def run_window_convolve(sample,y, W_matrices,bias):
    ...
    for i in range(WOMC.nlayer):
        ...
        for img in sample_b:
          ...
          Wsample_k.append(img_c.astype(jnp.int8))
        sample_b = jnp.array(Wsample_k)
    W_hood = jnp.array(sample_b[:, WOMC.ci_h, WOMC.ci_w]).astype(jnp.int8)
...
def MSE(true,pred):
  ...
  pred_ = jnp.where(pred == -1, 0, pred)
  return jnp.mean((true - pred_)**2)
...
calculate_neighbors_vmap = jax.vmap(calculate_neighbors_jax0, in_axes=(None, None, None,None, None,None,0))
...
def batch_run_(b,joint,joint_shape, W_matrices,W_matrices_all,train_b,ytrain_b,bias):
...
def batch_run(b,joint,joint_shape, W_matrices,W_matrices_all,train_b,ytrain_b,bias):
    ...
    joint_temp = []
    for k in range(WOMC.nlayer):
      if not WOMC.neighbors_sample_f or WOMC.neighbors_sample_f >= joint_shape[k]:
          neighbors_to_visit=jnp.array(range(joint_shape[k]))
      else:
          sample_indices = random.sample(range(joint_shape[k]), WOMC.neighbors_sample_f)
          neighbors_to_visit=jnp.array(sample_indices)
      result = calculate_neighbors_jax(joint, W_matrices_all,k, train_b[b],ytrain_b[b],bias,neighbors_to_visit)
      error.append(result[0])
      joint_temp.append(result[1])

    error_ep_f_hist = jnp.concatenate(error)
    ix = jnp.argmin(error_ep_f_hist)
    joint_f_hist = jnp.concatenate(joint_temp)
    return joint_f_hist[ix]
...
def _save_results_in(W, img_type, k, ep = None):
    W_txt = jax.device_get(W) 
    pickle.dump(W_txt, open(f'{work_output}/{WOMC.path_results}/run/{img_type}_{WOMC.name_save}_{ep}.txt', 'wb'))
...
def _save_results(W, img_type, k):
    W_txt = jax.device_get(W) 
    pickle.dump(W_txt, open(f'{work_output}/{WOMC.path_results}/trained/{img_type}_{WOMC.name_save}.txt', 'wb'))
...
def _save_results_fixed(W, img_type, k):
    W_txt = jax.device_get(W)  
    pickle.dump(W_txt, open(f'{work_output}/{WOMC.path_results}/trained/{img_type}_{WOMC.name_save}_fixed.txt', 'wb'))
...
def save_results_complet_fixed(Wtrain, Wval, Wtest):
    ...
    pickle.dump(Wtrain, open(name_file, 'wb'))
